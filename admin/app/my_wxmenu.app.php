...
class My_wxmenuApp extends BackendApp {
    ...
    private function getmenu() {
        ...
        $topmemu = $this->my_wxmenu_mod->find(array(
            'fields' => 'id, name, weixin_key as wkey, weixin_keyword as keyword,weixin_type as type,likes as url',
        ...
        foreach ($topmemu as $key) {
            ...
            if (count($nextmenu) != 0) {//没有下级栏目
                foreach ($nextmenu as $key2) {
                    if($key2['type']=='1')
                    {
                         $kk[] = array('type' => 'view', 'name' => urlencode($key2['name']), 'url' => urlencode($key2['url']));
                    }else{
                        $kk[] = array('type' => 'click', 'name' => urlencode($key2['name']), 'key' => urlencode($key2['wkey']));
                    }
                }
                ...
                if($key['type']=='1')
                {
                    $keyword['button'][] = array('type' => 'view', 'name' => urlencode($key['name']), 'url' => urlencode($key['url']));
                }else{
                     $keyword['button'][] = array('type' => 'click', 'name' => urlencode($key['name']), 'key' => $key['wkey']);
                }
            }
        }
    ...
    public function create_weixin_menu() {
        ...
        if(empty($wxconfig['access_token']) || intval($wxconfig['access_expire'])<gmtime())
        {
            $ACCESS_LIST = Init_Weixin::curl($appid, $appsecret); //获取到的凭证
            if(!empty($ACCESS_LIST['access_token']))
            {
                $this->my_wxconfig_mod->edit('user_id='. $this->_store_id,array('access_token'=>$ACCESS_LIST['access_token'],'access_expire'=>intval($ACCESS_LIST['expires_in'])+gmtime()));
            }
             
        }else{
           $ACCESS_LIST=array('access_token'=> $wxconfig['access_token']);
        }   

        //var_dump($ACCESS_LIST);exit;
        ...
        if ($ACCESS_LIST['access_token'] != '') {
            ...
            $data = $this->getmenu();
            $data=urldecode($data);
            $msg = Init_Weixin::curl_menu($access_token, $data);
            //var_dump($access_token);var_dump($data);exit;
            if ($msg['errmsg'] == 'ok') {
    ...
    function edit() {
        ...
        if (!IS_POST) {
            ...
            $data = array(
                ...
                'add_time' => gmtime(),
                'items' => 0,
                'likes' => !empty($_POST['likes']) ? trim($_POST['likes']) : '',
                'weixin_type' => isset($_POST['weixin_type']) ? intval($_POST['weixin_type']) : 0,
                'ordid' => 255,
                'weixin_status' => isset($_POST['status']) ? intval($_POST['status']) : 0,
