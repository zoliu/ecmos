...
class UserApp extends BackendApp
{
    ...
    function index()
    {
        ...
        foreach ($users as $key => $val)
        {
            ...
            //---www.360cd.cn  Mosquito---
            //用户等级
            $temp = $this->grade_model->get($val['grade_id']);
            $users[$key]['grade_name'] = $temp['grade_name'];
...
//            'phone_mob' => LANG::get('phone_mob'),
    ...
    function add()
    {
        if (!IS_POST)
        {
            ...
            //---www.360cd.cn  Mosquito---
            $this->assign('grade_options', UserGrade::init()->get_grade_options());
...
//                'im_aliww'  => $_POST['im_aliww'],
            ...
            $data['grade_id'] = intval($_POST['grade_id']) ? intval($_POST['grade_id']) : $init_grade['id'];
            ...
            User::init()->insert_user_data($data);
    ...
    function edit()
    {
        ...
        if (!IS_POST)
        {
            ...
            //---www.360cd.cn  Mosquito---
            //用户等级
            $temp = $this->grade_model->get($user['grade_id']);
            $user['grade_name'] = $temp['grade_name'];
