...
class RappModule extends AdminbaseModule {
	...
	function update_appcenter() {
		...
		$zip_name = SL('dir')->create_dir(ROOT_PATH . '/data/app/download/') . "rapp.zip";
		if (is_file($zip_name)) {
			@unlink($zip_name);
		}
		...
		if (is_dir(ROOT_PATH . '/data/app/test/rapp')) {
			SL('dir')->del_dir(ROOT_PATH . '/data/app/test/rapp');
		}

		$zip = new PclZip($zip_name);
		$zip->extract(PCLZIP_OPT_PATH, SL('dir')->create_dir(ROOT_PATH . '/data/app/test/rapp'));
		if ($zip->errorCode() != 0) {
		...
		//拷贝到根目录
		SL('dir')->copy_dir(ROOT_PATH . '/data/app/test/rapp', ROOT_PATH);
	...
	function app_install() {
		...
		if ($data['done']) {
			...
			$zip_name = SL('dir')->create_dir(ROOT_PATH . '/data/app/upload/') . "{$app_id}-{$version_id}.zip";
			if (is_file($zip_name)) {
				@unlink($zip_name);
			}
			$zip = new PclZip($zip_name);
		...
		//安装成功获取文件
		$file = file_get_contents($retval['url']);
		$zip_name = SL('dir')->create_dir(ROOT_PATH . '/data/app/download/') . "{$app_id}-{$version_id}.zip";
		if (is_file($zip_name)) {
			@unlink($zip_name);
		}
		file_put_contents($zip_name, $file);
		...
		if (is_dir(ROOT_PATH . '/data/app/test' . "/{$app_id}-{$version_id}")) {
			SL('dir')->del_dir(ROOT_PATH . '/data/app/test' . "/{$app_id}-{$version_id}");
		}
		$zip = new PclZip($zip_name);
		$zip->delete(PCLZIP_OPT_BY_NAME, 'sql/');
		$zip->extract(PCLZIP_OPT_PATH, SL('dir')->create_dir(ROOT_PATH . '/data/app/test' . "/{$app_id}-{$version_id}"));
		if ($zip->errorCode() != 0) {
			show_warning('解压下载文件失败');
			exit();
		}
		...
		//拷贝到根目录
		SL('dir')->copy_dir(ROOT_PATH . '/data/app/test' . "/{$app_id}-{$version_id}", ROOT_PATH);
	...
	function app_uninstall() {
		...
		SL('dir')->del_empty_dir(ROOT_PATH . '/external');
