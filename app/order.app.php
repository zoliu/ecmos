...
class OrderApp extends ShoppingbaseApp
{
    ...
    function index()
    {
        ...
        import('zllib/order.lib');
        $order_lib=new orderBase();
        ...
        if (!IS_POST)
         {
            ...
            foreach ($carts as $key => $goods_info) {
                ...
                $carts[$key]['shipping_methods']=$order_lib->_get_shipping($form['data']['shipping_methods'],$goods_info['quantity']);
            }
            ...
            // 得到所有配送方式名称
            $this->assign('options_shipping',$order_lib->_get_shipping_name());
        ...
        else
        {
            ...
            if($update_order_result == 20){
                foreach ($orders as $key => $order_id) {
                    ...
                    $this->show_message("order_commit_yes",
                    "back_buyer_order",'index.php?app=buyer_order');
                }
                
            }else{
                //360cd.cn
                 /*到收银台付款 */
                $order_id =$order_lib->get_order_id($orders);//返回订单id 360cd.cn  seema
            ...
            default:
                ...
                $cart_model =& m('cart');
                    ...
                    $cart_items      =  $cart_model->find(array(
                        'conditions' => "user_id = " . $this->visitor->get('user_id') . " AND rec_id in ({$rec_ids}) and store_id={$store_id} AND session_id='" . SESS_ID . "'",
